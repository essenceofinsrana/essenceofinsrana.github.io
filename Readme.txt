...
│  [Technical files]
├── style.css
├── loadcss.js (for html pages only)
├── robots.txt
├── sitemap_map1.xml
│
│  [Information files]
...
====Webpages===
WCAG 2.2 AA compliant to a large extent
Skip to main button
...
Carousel image dots if clicked/ tapped result in manual navigation and pausing of auto-scroll for a period of 10 secs.
...
Toggle label changes to hide or show caption and zoom image or reset zoom respectively
...
Grab and Grabbing of mouse pointer when image
Semi-transparent background of controls
Large touch-areas for controls
Changing image resets zoom, closing lightbox resets zoom, open lightbox resets zoom
...
B. Circular Indexing & Normalised Positioning Logic
...
C. Timer Coordination & User Override Logic

The automatic slider rotation is governed by an asynchronous loop coupled with a temporary override fallback:

1.  Standard Loop: Calls goNext() every 3,000ms via setInterval.
2.  Hover Pause: Pauses the timer on cursor entry. Crucially, the logic filters input devices by checking e.pointerType === "mouse". Touch-tap interactions are excluded, preventing the slideshow from freezing on mobile devices.
3.  Manual Override: Clicking or swiping triggers pauseForManualNavigation(), which executes the following sequence:
...
D. Roving Tabindex for Control Dots
...
B. Double-Buffered Slide Transitions
...
1.  Off-Screen Prep: When navigating to another slide, the incoming asset is rendered inside a hidden image node (standbyImage) and placed off-screen via CSS translation transforms (.from-right or .from-left).
2.  Framerate Synchronisation: Nested requestAnimationFrame hooks ensure style updates register with the browser's paint cycle before introducing transition styles.
3.  Active Transition: The current active image (activeImage) is translated out of view, and the standbyImage moves into the active viewport coordinates (.centre).
4.  Reference Swap: Once the transitions complete, the DOM object references are swapped:
    const temp = activeImage;
    activeImage = standbyImage;
    standbyImage = temp;
    standbyImage.src = ""; // Flushes standby memory to free resources

C. Intelligent Image Preloading Heuristics
...
D. Dynamic Zoom Transformation Origin
...
E. Advanced Pointer Drag, Boundary Physics, and Momentum Engine
...
1. Real-Time Tracking & Elastic Boundary Rubber-Banding
...
2. Momentum Deceleration Mechanics
